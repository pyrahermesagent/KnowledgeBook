...
async function runAllBenchmarks() {
  ...
  for (const result of latencyResults) {
    ...
    const regression = (
      ((result.avgLatency - baseline.avgLatency) / baseline.avgLatency) *
      100
    ).toFixed(1);
  ...
  for (const category of ['latency', 'throughput', 'memory']) {
    for (const result of allResults[category]) {
      if (
        ...
        parseFloat(result.regressionPercent) <=
          (REGRESSION_THRESHOLDS[category === 'latency' ? 0 : category === 'throughput' ? 2 : 1] ===
          10
            ? 10
            : category === 'throughput'
              ? 15
