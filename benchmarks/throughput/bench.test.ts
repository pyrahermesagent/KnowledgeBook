...
describe('Throughput Benchmarks', () => {
  ...
  it('should handle 100 concurrent project creations', async () => {
    ...
    const stmt = db.prepare(
      'INSERT INTO projects (owner_id, slug, name, description) VALUES (?, ?, ?, ?)'
    );
    ...
    for (let batch = 0; batch < numBatches; batch++) {
      ...
      for (let i = 0; i < batchSize; i++) {
        ...
        promises.push(
          stmt.run(
            user.id,
            `throughput-${projectNum}`,
            `Throughput Project ${projectNum}`,
            'Throughput test'
          )
        );
      }
    ...
    console.log(
      `Throughput: ${throughput} projects/second (${duration.toFixed(2)}ms for 100 projects)`
    );
  ...
  it('should handle concurrent page reads', async () => {
    ...
    console.log(
      `Concurrent reads: ${throughput} reads/second (${duration.toFixed(2)}ms for 50 reads)`
    );
  ...
  it('should handle concurrent writes with transaction', async () => {
    ...
    console.log(
      `Concurrent writes (transaction): ${throughput} writes/second (${duration.toFixed(2)}ms for 30 writes)`
    );
