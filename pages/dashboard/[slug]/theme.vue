      ...
      <div class="theme-preview-box" :style="previewStyle">
        ...
        <div class="preview-text-block">
          <p class="preview-text">
            Example paragraph with <a href="#" class="preview-link">link</a> text.
          </p>
...
const { data: themeData } = await useFetch(`/api/projects/${slug.value}/theme`);
onMounted(() => {
  if (themeData.value) {
...
.theme-admin-header h2 {
  margin: 0 0 8px;
  font-size: 26px;
}
.theme-admin-header .muted {
  margin: 0;
  font-size: 15px;
}
