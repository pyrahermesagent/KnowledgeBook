...
export default defineEventHandler(async (event) => {
  ...
  const page = db
    ...
    | { id: number; section_id: number | null; title: string; content: string; position: number }
    | undefined;
  ...
  if (
    sectionId != null &&
    !db.prepare('SELECT 1 FROM sections WHERE id = ? AND project_id = ?').get(sectionId, project.id)
  ) {
