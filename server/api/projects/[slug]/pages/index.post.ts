export default defineEventHandler(async (event) => {
  ...
  for (
    let n = 2;
    db.prepare('SELECT 1 FROM pages WHERE project_id = ? AND slug = ?').get(project.id, slug);
    n++
  ) {
    slug = `${base}-${n}`;
  }
  ...
  const { max } = db
    .prepare(
      'SELECT COALESCE(MAX(position), -1) AS max FROM pages WHERE project_id = ? AND section_id IS ?'
    )
