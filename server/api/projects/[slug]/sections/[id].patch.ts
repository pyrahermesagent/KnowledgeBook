export default defineEventHandler(async (event) => {
  ...
  const db = useDb();
  const section = db
    .prepare('SELECT id, title, position FROM sections WHERE id = ? AND project_id = ?')
  ...
  db.prepare('UPDATE sections SET title = ?, position = ? WHERE id = ?').run(
    title,
    body.position ?? section.position,
    id
  );
  return { ok: true };
});
