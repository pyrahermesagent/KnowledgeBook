export default defineEventHandler(async (event) => {
  ...
  if (!slug)
    throw createError({ statusCode: 400, message: 'Could not derive a valid link from that name' });
  if (RESERVED_SLUGS.has(slug))
    throw createError({ statusCode: 400, message: `"${slug}" is a reserved link` });
  if (getProjectBySlug(slug))
    throw createError({ statusCode: 409, message: `The link /${slug} is already taken` });
  ...
  const section = db
    .prepare('INSERT INTO sections (project_id, title, position) VALUES (?, ?, 0)')
    .run(projectId, 'Getting started');
  db.prepare(
    'INSERT INTO pages (project_id, section_id, slug, title, content, position) VALUES (?, ?, ?, ?, ?, 0)'
  ).run(
    projectId,
    Number(section.lastInsertRowid),
    'introduction',
    'Introduction',
