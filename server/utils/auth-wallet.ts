...
export async function verifyLoginAttempt (
  ...
  if (address !== parsed.address.toLowerCase()) {
    return { success: false, address: '', reason: 'Signature does not match the address in the message' }
  }
...
export function upsertWalletUser (address: string, chainId: number): number {
  ...
  const existing = db.prepare('SELECT id FROM wallet_users WHERE wallet_address = ?').get(walletAddress) as { id: number } | undefined
