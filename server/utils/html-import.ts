...
export function convertHtmlToMarkdown(html: string, _options: ImportOptions = {}): string {
  let content = html;
...
export function parseHtmlDirectory(
  basePath: string,
  htmlFiles: string[]
): { pages: ImportPage[]; metadata: ImportMetadata } {
  const pages: ImportPage[] = [];
  const metadata: ImportMetadata = {};
...
export async function importHtmlContent(
  ...
  try {
    ...
    if (content.trim().startsWith('<?xml') || content.trim().startsWith('<')) {
      ...
      if (
        lowerContent.includes('wordpress') ||
        ...
        lowerContent.includes('mediawiki') ||
        lowerContent.includes('siteinfo') ||
