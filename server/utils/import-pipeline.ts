...
export interface ImportOptions {
  ...
  projectName?: string;
  format?:
    'auto' | 'gitbook' | 'markdown' | 'html' | 'csv' | 'confluence' | 'notion' | 'pdf' | 'epub';
  url?: string;
  content?: string;
...
export function detectFormat(
  ...
  url?: string,
  filename?: string
): 'gitbook' | 'markdown' | 'html' | 'csv' | 'confluence' | 'notion' | 'pdf' | 'epub' {
...
async function importMarkdownPipeline(
  ...
  const projectSlug =
    options.projectSlug || uniqueProjectSlug(options.projectName || 'Markdown Import');
...
async function importConfluencePipeline(
  options: ImportOptions & { content: string }
): Promise<ImportResult> {
  const db = useDb();

  const projectSlug =
    options.projectSlug || uniqueProjectSlug(options.projectName || 'Confluence Import');
...
async function importNotionPipeline(
  options: ImportOptions & { content: string }
): Promise<ImportResult> {
  const db = useDb();

  const projectSlug =
    options.projectSlug || uniqueProjectSlug(options.projectName || 'Notion Import');
...
async function importPdfPipeline(
  options: ImportOptions & { content: string; filename?: string }
): Promise<ImportResult> {
  const db = useDb();
...
async function importEpubPipeline(
  options: ImportOptions & { content: string }
): Promise<ImportResult> {
  const db = useDb();

  const projectSlug =
    options.projectSlug || uniqueProjectSlug(options.projectName || 'EPUB Import');
  ...
  switch (detectedFormat) {
    ...
    case 'confluence':
      if (!content) {
        throw createError({
          statusCode: 400,
          message: 'Content is required for Confluence import',
        });
      }
    ...
    case 'pdf':
      if (!content && !options.file) {
        throw createError({
          statusCode: 400,
          message: 'Content or file is required for PDF import',
        });
      }
      return await importPdfPipeline({
        ownerId,
        content: content || '',
    ...
    case 'epub':
      if (!content && !options.file) {
        throw createError({
          statusCode: 400,
          message: 'Content or file is required for EPUB import',
        });
      }
      return await importEpubPipeline({
        ownerId,
        content: content || '',
        filename: options.file?.name,
      });
...
export function validateImportRequest(options: ImportOptions): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
