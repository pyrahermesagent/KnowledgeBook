...
export async function validateTokenAccess(
  ...
  const tokenProject = db
    .prepare('SELECT * FROM token_gated_projects WHERE project_id = ?')
...
export async function getWalletTokenProjects(walletAddress: string): Promise<number[]> {
  ...
  const gatedProjects = db.prepare('SELECT project_id FROM token_gated_projects').all() as {
    project_id: number;
  }[];
