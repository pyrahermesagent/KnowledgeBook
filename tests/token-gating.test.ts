...
function seedProjects(count = 2) {
  ...
  for (let i = 1; i <= count; i++) {
    db.prepare('INSERT INTO projects (owner_id, slug, name) VALUES (1, ?, ?)').run(`p${i}`, `P${i}`)
  }
  return db
}
...
describe('validateTokenAccess', () => {
  ...
  it('takes the ERC-721 branch when min_balance is SQL NULL', async () => {
    ...
    const selectors = fetchMock.mock.calls.map(
      ([, init]: any) => JSON.parse(init.body).params[0].data.slice(0, 10)
    )
...
describe('NFT project ownership', () => {
  ...
  it('upserts rather than duplicating on re-registration', async () => {
    ...
    const rows = db.prepare('SELECT * FROM nft_project_ownership WHERE project_id = 1').all() as any[]
  ...
  describe('transferProjectOwnershipViaNft', () => {
    ...
    it('moves the project and the ownership record together', async () => {
      ...
      const project = db.prepare('SELECT owner_wallet_address FROM projects WHERE id = 1').get() as any
